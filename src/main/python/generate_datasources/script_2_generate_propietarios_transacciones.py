...
# for _, row in df_mascotas.iterrows():
#     try:
...
#             # Random service selection, with "Eutanasia" having a small probability
#             if not pet_has_euthanasia and random.random() < 0.05:  # 5% chance for "Eutanasia"
#                 servicio_prestado = EUTANASIA
#                 pet_has_euthanasia = True  # Ensure no more services for this pet
#             else:
...
#         # If "Eutanasia" was assigned, ensure it has the latest date
#         if pet_has_euthanasia:
#             # Find the Eutanasia row
#             euthanasia_row = next((t for t in pet_transactions if t["Servico Prestado"] == EUTANASIA), None)
#             if euthanasia_row:
#                 # Set "Eutanasia" to the latest date
#                 max_date = max(t["Fecha del Servicio"] for t in pet_transactions)
#                 euthanasia_row["Fecha del Servicio"] = max_date
